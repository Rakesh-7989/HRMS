...
export const DesignationsContent: React.FC = () => {
    ...
    const handleDelete = async (id: string) => {
        ...
        const result = await confirm({
            ...
            message: t('designations.deleteMessage', { name: desig?.name || '' }),
            type: 'destructive',
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">{t('designations.title')}</h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        {t('designations.description')}
                    </p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <div className="relative hidden sm:block">
                        <input
                            type="text"
                            placeholder={t('designations.searchPlaceholder')}
            ...
            {isLoading ? (
                <div className="h-64 flex flex-col items-center justify-center">
                    ...
                    <p className="text-muted text-sm">{t('designations.loading')}</p>
                </div>
                ...
                <Card className="py-16 flex flex-col items-center justify-center text-center">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-1">
                        {searchTerm ? t('designations.noMatching') : t('designations.noDesignations')}
                    </h3>
                    <p className="text-gray-500 dark:text-gray-400 max-w-sm mx-auto mb-6">
                        {searchTerm
                            ? t('designations.noMatchingDesc', { searchTerm })
                            : t('designations.noDesignationsDesc')}
                    </p>
                </Card>
            ) : (
                        ...
                        <Card key={d.id} className="group hover:shadow-elev-3 transition-all duration-200 border-gray-200/60 dark:border-gray-700">
                            <div className="p-4" >
                                ...
                                <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 h-10 mb-3" title={d.description || ''}>
                                    {d.description || t('designations.noDescription')}
                                </p>
