...
export const TaskComments: React.FC<TaskCommentsProps> = ({ taskId, projectId }) => {
    ...
    const renderCommentContent = (content: string) => {
        ...
        return parts.map((part, i) => {
            ...
            if (i % 2 === 1) {
                return (
                    <span key={i} className="text-blue-400 dark:text-blue-300 font-medium">
                        @{part}
                    </span>
                );
            }
                            ...
                            {editingId === comment.id ? (
                                <div className="space-y-2">
                                    <textarea
                                        ...
                                        className="w-full p-2 text-sm border border-gray-300 dark:border-gray-600 rounded-md resize-none bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                                        rows={2}
                                    />
            ...
            <form onSubmit={handleSubmit} className="relative">
                <div className="relative" ref={mentionsRef}>
                    <textarea
                        ...
                        className="w-full p-3 pr-12 text-sm border border-gray-300 dark:border-gray-600 rounded-lg resize-none bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                        rows={2}
                    />
                    <button
                        type="submit"
                        disabled={!newComment.trim() || createMutation.isPending}
                        className="absolute right-2 bottom-2 p-2 text-blue-500 dark:text-blue-400 hover:bg-blue-500/10 rounded-md disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    >
