-- Migration: Comprehensive Payroll Schema Fix
-- Date: 2026-01-09
...
DO $$
BEGIN
    EXECUTE 'ALTER TABLE pay_schedules ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE salary_templates ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE statutory_config ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE pt_slabs ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE deduction_types ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE expense_categories ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE employee_expenses ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE employee_salary_details ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE salary_revisions ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE employee_loans ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE employee_loan_installments ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE payroll_runs ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE payroll_run_items ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE employee_tax_declarations ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE holidays ENABLE ROW LEVEL SECURITY';
EXCEPTION
    WHEN OTHERS THEN NULL;
END $$;

-- Policies (using dynamic SQL to avoid errors if policies already exist)
DO $$
DECLARE
    t text;
BEGIN
    FOREACH t IN ARRAY ARRAY[
        'pay_schedules', 'salary_templates', 'statutory_config', 'pt_slabs',
        'deduction_types', 'expense_categories', 'employee_expenses',
        'employee_salary_details', 'salary_revisions', 'employee_loans',
        'employee_loan_installments', 'payroll_runs', 'payroll_run_items',
        'employee_tax_declarations', 'holidays'
    ]
    LOOP
        BEGIN
            EXECUTE format('CREATE POLICY %I_isolation ON %I USING (tenant_id = current_tenant())', t, t);
        EXCEPTION
