...
const getReviewData = async (tenantId, runId) => {
    ...
    // Exit employees (resigned/terminated during this period)
    ...
    try {
        const statRes = await db.query(
            `SELECT pf_enabled FROM statutory_config WHERE tenant_id = $1`, [tenantId]
        );
    ...
    // Manage Checklist DB Entries
    const existingChecks = await db.query(
        `SELECT * FROM payroll_checklist WHERE payroll_run_id = $1`, [runId]
    );

    if (existingChecks.rows.length === 0) {
        for (const check of systemChecks) {
            await db.query(
                `INSERT INTO payroll_checklist (payroll_run_id, item_name, status, comment) VALUES ($1, $2, $3, $4)`,
                [runId, check.name, check.status, check.comment]
            );
        }
    } else {
        for (const check of systemChecks) {
            await db.query(
                `UPDATE payroll_checklist SET status = $1, comment = $2 WHERE payroll_run_id = $3 AND item_name = $4`,
                [check.status, check.comment, runId, check.name]
            );
        }
    }

    const checklistRes = await db.query(
        `SELECT * FROM payroll_checklist WHERE payroll_run_id = $1 ORDER BY id ASC`, [runId]
    );
    ...
    const grossDiff = curGross - prevGross;
    ...
    const variance = {
        gross: { current: curGross, previous: prevGross, diff: grossDiff, percent: prevGross === 0 ? 0 : ((grossDiff / prevGross) * 100).toFixed(2) },
    ...
    return {
        ...
        categories: {
            headcount: {
                ...
                newJoinerDetails: newJoiners,
                exitDetails: exitEmployees
            },
            attendance: {
                employeesWithLop: lopEmployees.length,
                lopDetails: lopEmployees
            },
            finance: {
                salaryRevisions: salaryRevisions.length,
                totalTdsEmployees,
                revisionDetails: salaryRevisions
            }
        }
    };
};
        ...
        for (const item of items) {
            ...
            try {
                ...
                for (const comp of compRes.rows) {
                    ...
                    if (compName.includes('BASIC')) basicPay = amt;
                    else if (compName.includes('HRA')) hra = amt;
                    else if (compName.includes('PF') && compName.includes('EMPLOYEE')) pfEmp = amt;
                    else if (compName.includes('PF') && compName.includes('EMPLOYER')) pfEr = amt;
                    else if (compName.includes('ESI') && compName.includes('EMPLOYEE')) esiEmp = amt;
                    else if (compName.includes('ESI') && compName.includes('EMPLOYER')) esiEr = amt;
                    else if (compName.includes('PT') || compName.includes('PROFESSIONAL')) pt = amt;
                    else if (compName.includes('TDS') || compName.includes('INCOME TAX')) tds = amt;
