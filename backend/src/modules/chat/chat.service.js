    ...
    SELECT 
      ...
      (
        ...
        WHERE m.conversation_id = c.id 
          AND NOT EXISTS (SELECT 1 FROM message_user_deletions mud WHERE mud.message_id = m.id AND mud.user_id = $1)
        ORDER BY m.created_at DESC LIMIT 1
      ) as last_message,
      (
        SELECT json_agg(json_build_object('id', u.id, 'user_id', u.id, 'email', u.email, 'first_name', emp.first_name, 'last_name', emp.last_name, 'profile_pic', emp.profile_photo_url, 'employee_id', emp.employee_id, 'status', u.status, 'status_message', u.status_message, 'status_expiry', u.status_expiry))
...
exports.getConversation = async (db, conversationId, userId) => {
  ...
  const res = await query(
    ...
    SELECT 
      ...
      cp.unread_count,
      (
        SELECT json_agg(json_build_object('id', u.id, 'email', u.email, 'first_name', emp.first_name, 'last_name', emp.last_name, 'profile_pic', emp.profile_photo_url, 'employee_id', emp.employee_id, 'status', u.status, 'status_message', u.status_message, 'status_expiry', u.status_expiry))
        FROM conversation_participants part
        JOIN users u ON part.user_id = u.id
        LEFT JOIN employees emp ON u.id = emp.user_id
...
exports.getMessages = async (db, conversationId, userId, limit = 50, offset = 0) => {
  ...
  const res = await query(
    ...
    SELECT 
      ...
      e.profile_photo_url as sender_profile_pic,
      e.employee_id as sender_employee_id,
      (
    ...
    WHERE m.conversation_id = $1 
      AND NOT EXISTS (
        ...
        WHERE mud.message_id = m.id AND mud.user_id = $4
      )
...
exports.sendMessage = async (db, senderId, tenantId, conversationId, content, type = 'TEXT', fileUrl = null, parentId = null) => {
  ...
  try {
    ...
    const convRes = await query(
      `SELECT c.id, c.name, c.type FROM conversations c WHERE c.id = $1`,
      [conversationId]
    );
    const convInfo = convRes.rows[0];
    ...
    const senderName = `${fullMessage.sender_first_name || ''} ${fullMessage.sender_last_name || ''}`.trim() || 'Someone';
    ...
    participants.rows.forEach(p => {
      ...
      if (p.user_id !== senderId) {
        ...
        // Send rich notification for global chat notification banner
        io.to(`user_${p.user_id}`).emit("chat_notification", {
          messageId: fullMessage.id,
          conversationId,
          conversationName: convInfo?.name || null,
          conversationType: convInfo?.type || 'DIRECT',
          senderId,
          senderName,
          senderAvatar: fullMessage.sender_profile_pic || null,
...
exports.deleteMessage = async (db, userId, conversationId, messageId, mode = 'everyone') => {
  const query = queryDb(db);

  if (mode === 'me') {
    await query(
      `INSERT INTO message_user_deletions (message_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
      [messageId, userId]
    );
  ...
  const msgRes = await query(`SELECT sender_id FROM messages WHERE id = $1 AND conversation_id = $2`, [messageId, conversationId]);
  if (msgRes.rowCount === 0) throw new NotFoundError("Message not found");
  if (msgRes.rows[0].sender_id !== userId) throw new ForbiddenError("You can only delete for everyone on your own messages");
...
exports.getContacts = async (db, userId, tenantId) => {
  ...
  const res = await query(`
        SELECT u.id, u.email, e.first_name, e.last_name, d.name as designation, e.employee_id, e.profile_photo_url as profile_pic
        FROM users u
        LEFT JOIN employees e ON u.id = e.user_id
...
exports.addParticipants = async (db, conversationId, userIds, requesterId) => {
  const query = queryDb(db);

  // Verify conversation is a GROUP and requester is a participant (and optionally admin)
  const convRes = await query(
  ...
  if (convRes.rowCount === 0) {
    throw new ForbiddenError("You do not have permission to modify this group");
  }

  if (convRes.rows[0].type !== 'GROUP') {
    throw new BadRequestError("Cannot add participants to a direct conversation");
  }

  // Filter out users who are already participants
  const existingPartsRes = await query(
    `SELECT user_id FROM conversation_participants WHERE conversation_id = $1`,
    [conversationId]
  );
  const existingUserIds = existingPartsRes.rows.map(r => r.user_id);
  const newUsers = userIds.filter(uid => !existingUserIds.includes(uid));

  if (newUsers.length === 0) {
    return await exports.getConversation(db, conversationId, requesterId);
  }
  ...
  try {
    const io = getIo();
    io.to(conversationId).emit("participants_added", { conversationId, addedUserIds: newUsers });

    // Also notify the new users specifically so they see the conversation in their list
...
exports.removeParticipant = async (db, conversationId, targetUserId, requesterId) => {
  const query = queryDb(db);

  // Check permissions: Requester must be admin OR leaving themselves
  const requesterRes = await query(
    `SELECT cp.is_admin 
     FROM conversation_participants cp 
     JOIN conversations c ON cp.conversation_id = c.id
     WHERE cp.conversation_id = $1 AND cp.user_id = $2 AND c.type = 'GROUP'`,
    [conversationId, requesterId]
  );

  if (requesterRes.rowCount === 0) {
    throw new ForbiddenError("You are not part of this group");
  }

  const isAdmin = requesterRes.rows[0].is_admin;
  const isLeaving = String(targetUserId) === String(requesterId);

  // Temporarily allowing any member to remove for flexibility or restrict to admin
  // For better UX, usually only Admin can remove others. But let's check basic rule.
  if (!isLeaving && !isAdmin) {
    // Check if there are ANY admins. If no admins exist in group (edge case), maybe allow?
    // But standard is only admin can remove others.
    // Let's enforce Admin only for removing others.
    throw new ForbiddenError("Only group admins can remove other participants");
  }
...
exports.updateStatusMessage = async (db, userId, message, expiry) => {
  const query = queryDb(db);
  await query(`UPDATE users SET status_message = $1, status_expiry = $2 WHERE id = $3`, [message, expiry, userId]);
  return { userId, message, expiry };
};
...
exports.clearChatHistory = async (db, conversationId, userId) => {
  const query = queryDb(db);
  ...
  if (partRes.rowCount === 0) throw new ForbiddenError("You are not a participant in this conversation");

  // Soft delete all messages for this user by inserting into message_user_deletions
  await query(
  ...
  // Reset unread counts for THIS user only
  await query(`UPDATE conversation_participants SET unread_count = 0 WHERE conversation_id = $1 AND user_id = $2`, [conversationId, userId]);

  // Real-time notify only the user who cleared the chat
  try {
    const io = getIo();
...
exports.deleteConversation = async (db, conversationId, userId) => {
  const query = queryDb(db);
  ...
  const participantsRes = await query(`SELECT user_id FROM conversation_participants WHERE conversation_id = $1`, [conversationId]);

  // HARD delete conversation (cascades to participants and messages)
  await query(`DELETE FROM conversations WHERE id = $1`, [conversationId]);

  // Real-time notify all participants to remove the conversation from their UI
  try {
    const io = getIo();
